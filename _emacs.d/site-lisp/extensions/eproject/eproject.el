...
(defgroup prj-group '()
   "Group for eproject customization")


(defcustom prj-keybindings '(
...
(defun prj-walk-path (dir action)
	     ...
	     (cond ((member file '("." "..")))
		   (t
		    (and (funcall action dir file)
			 ...
			 (prj-walk-path fullname action)))))))
	(t
	 (funcall action
		  (file-name-directory dir)
		  (file-name-nondirectory dir)))))

(defun prj-add-if (p dir file)
